...
class OrganIKB(BotIKB):    
    ...
    def info(self, data: AnswerOrganInfo, where: str | None = None):
        ...
        elif not data.for_buttons.is_capture:
            self.builder.button(text='👑 Захватить власть', callback_data=OrganCaptureCall(organ_id=data.organ.id, tg_id=self.tg_id))
        if where:
    ...
    def organs(self, organs: list[OrganInfo], page: int, max_page: int, where: str | None = None):
        for organ in organs:
            self.builder.button(text=f'{organ.emodzi or '🏛'} {organ.name}', icon_custom_emoji_id=organ.custom_emodzi_id, callback_data=OrganInfoCall(organ_id=organ.id, tg_id=self.tg_id))
        ...
        if where:
            self.builder.row(InlineKeyboardButton(text='✒️ Ввести запрос еще раз', callback_data=OrganBackCall(where=where, tg_id=self.tg_id).pack()))
    ...
    def members(self, members: list[AnswerMemberInfo], organ_id: int):
        for mbr in sorted(members, key=lambda x: x.member.rank):
            self.builder.button(text=f'{mbr.member.rank} - {mbr.user.fullname or mbr.user.fullname} ({mbr.member.rank_name})', callback_data=OrganMemberCall(purpose_tg_id=mbr.user.tg_id, tg_id=self.tg_id))
    ...
    def top(self, organs: list[OrganInfo]):
        for n, organ in enumerate(organs, start=1):
            self.builder.button(text=f'{n}. {organ.emodzi or '🏛'} {f'{organ.name[:15]}...' if len(organ.name) > 15 else f'{organ.name}'} ({organ.member_counts} участ.)', icon_custom_emoji_id=organ.custom_emodzi_id, callback_data=OrganInfoCall(organ_id=organ.id, tg_id=self.tg_id))
        return self.builder.adjust(1).as_markup()

    def give(self, purpose: AnswerMemberInfo, is_back: bool = False):
        self.builder.button(text='✅ Да', callback_data=OrganGiveCall(accert=True, purpose_tg_id=purpose.user.tg_id, tg_id=self.tg_id))
        if is_back:
            self.builder.button(text='❌ Нет', callback_data=OrganMemberCall(purpose_tg_id=purpose.user.tg_id, tg_id=self.tg_id))
        else:
            self.builder.button(text='❌ Нет', callback_data=OrganGiveCall(cancel=True, purpose_tg_id=purpose.user.tg_id, tg_id=self.tg_id))
        return self.builder.adjust(2).as_markup()