    ...
    async def wrapped(message: Message, **kwargs): 
        ...
        try:
            ...
            if message.from_user.id == settings.owner:
                ...
                await message.answer(f'{PythonError.msg}, Файл: {frame.filename}, строка: {frame.lineno}, функция: {frame.name}')
                await message.answer(f'{PythonError.msg}, Код: {frame.line}')
            else:
